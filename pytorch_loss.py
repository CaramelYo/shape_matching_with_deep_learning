...
# class ShapeMatchingLoss(torch.autograd.Function):
    ...
    def forward(self, pr_a, pr_b, input_a, input_b):
        ...
        a_0_pair = torch.cat((pr_a[0, 0, 0, 0:1], torch.zeros(1))).view(b, c, -1, 2)
        a_1_pair = torch.cat((pr_a[0, 0, 0, 1:2], torch.zeros(1))).view(b, c, -1, 2)
        b_0_pair = torch.cat((pr_a[0, 0, 0, 2:3], torch.zeros(1))).view(b, c, -1, 2)
        b_1_pair = torch.cat((pr_a[0, 0, 0, 3:4], torch.zeros(1))).view(b, c, -1, 2)

        input_a_x = input_a[:, :, 0:1, :]
        input_a_y = input_a[:, :, 1:2, :]

        start_a_x = torch.nn.functional.grid_sample(input_a_x, a_0_pair)
        start_a_y = torch.nn.functional.grid_sample(input_a_y, a_0_pair)
        end_a_x = torch.nn.functional.grid_sample(input_a_x, a_1_pair)
        end_a_y = torch.nn.functional.grid_sample(input_a_y, a_1_pair)

        selected_a = torch.cat((start_a_x, start_a_y), dim=3)
        ...
        while a_t < a_1:
            ...
            point_a = torch.cat((
                torch.nn.functional.grid_sample(input_a_x, a_t_pair),
                torch.nn.functional.grid_sample(input_a_y, a_t_pair)
            ), dim=3)

            selected_a = torch.cat((selected_a, point_a), dim=2)

            a_t = a_t + torch.tensor([0.001])

        selected_a = torch.cat((selected_a, torch.cat((end_a_x, end_a_y), dim=3)), dim=2)
